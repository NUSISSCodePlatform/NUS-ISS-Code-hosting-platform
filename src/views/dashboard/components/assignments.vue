        ...
        <div class="projects-section">
          ...
          <div>
            <div class="assignment-container">
              ...
              <el-button v-if="isAfterDeadline" @click="handleSubmit">Submit</el-button>
              <el-button :disabled="!isAfterDeadline">Submit</el-button>-->
...
export default {
  ...
  data() {
    return {
      ...
      dueDate: 'XXX',
      availableDate: 'XXX',
      url: 'https://github.com/WANGRUIJUN0804/ProjectA.git',
      userName: 'Jack',
  ...
  methods: {
    ...
    async fetchAssignmentInfo() {
      try {
        const token = localStorage.getItem('token'); // 假设JWT token保存在localStorage
        const course = this.$route.params.course;
        const response = await apiClient.get('/student/assignmentinfo', {
          headers: {
    ...
    async handleSubmit() {
      try {
        const token = localStorage.getItem('token'); // 假设JWT token保存在localStorage
        const course = this.$route.params.course;
        const response = await apiClient.post('/student/uploadurl', {
          course: course,
          url: this.url
        }, {
          headers: {
    ...
    goToKnowmore(courseName) {
      const currentPath = this.$route.path;
      this.$router.push(`${currentPath}/projectInfo`).catch(err => {
        console.error(err);
      });
    },
    jumpModify(name) {
    ...
    async fetchTime() {
      try {
        ...
        const response = await apiClient.get('/student/grouptime', {
          headers: {
            'Authorization': `Bearer ${token}`
          },
          params: {
        ...
        this.groupDDLtime = response.data.time;
      } catch (error) {
        console.error('Error fetching teams:', error);
      }
    },
    isAfterDeadline() {
      return this.groupDDLtime < this.currentTime;
    },
    goToCourses() {
